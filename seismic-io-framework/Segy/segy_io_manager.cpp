...
vector<uint> SEGYIOManager::GetUniqueOccurences(string file_name, string key_name, uint min_threshold, uint max_threshold) {
...
// object
void SEGYIOManager::ReadTracesDataFromFile(string file_name, string sort_type,
                                           SeIO *sio) {
  ...
  for (auto it = seg->traces.begin(); it != seg->traces.end(); it++, ix++) {
    ...
    sio->Atraces[ix].TraceMetaData.scalar = it->scalco;
...
void SEGYIOManager::ReadVelocityDataFromFile(string file_name, string sort_type,
                                             SeIO *sio) {
  ...
  sio->DM.nx = seg->ntraces();
  sio->DM.nz = seg->bh.hns;
  sio->DM.ny = 1;
  sio->DM.dx = (seg->traces.at(1).sx * dBtoscale(seg->traces.at(1).scalco)) -
               (seg->traces.at(0).sx * dBtoscale(seg->traces.at(0).scalco));
  sio->DM.dz = seg->bh.hdt / (float)1000.0;
  sio->DM.dy = 1.0;

  int ix = 0;

  //      cout << "ntraces " << seg->ntraces() << endl;

  //cout << "SIO dt " << sio->DM.dt << endl;

  // cout << "shot id " << seg->traces.begin()->fldr << endl;
  ...
  for (auto it = seg->traces.begin(); it != seg->traces.end(); it++, ix++) {
    ...
    sio->Velocity[ix].TraceMetaData.scalar = it->scalco;

    // cout << "SIO scalr " << it->fldr << endl;

    memcpy(sio->Velocity[ix].TraceData, it->data, 32767 * sizeof(float));
  }

  //   cout << " nx  in seg repo" <<  seg->bh.ntrpr << endl;
  delete (seg);
}

void SEGYIOManager::ReadDensityDataFromFile(string file_name, string sort_type,
                                            SeIO *sio) {
  ...
  for (auto it = seg->traces.begin(); it != seg->traces.end(); it++, ix++) {
    ...
    sio->Density[ix].TraceMetaData.scalar = it->scalco;
...
void SEGYIOManager::ReadSelectiveDataFromFile(string file_name,
                                              string sort_type, SeIO *sio,
                                              int cond) {
  ...
  for (auto it = seg->traces.begin(); it != seg->traces.end(); it++, ix++) {
    ...
    // cout << "value of rec location x at " << ix <<"  " <<
    // sio->Atraces[ix].TraceMetaData.receiver_location_x  << endl;

    memcpy(sio->Atraces[ix].TraceData, it->data, 32767 * sizeof(float));
  }

  // cout << "value of rec location x after loop " << (int
  // )sio->Atraces[1000].TraceMetaData.receiver_location_x  << endl;

  // cout << "value of rec location at 1000 " <<
  // sio->Atraces[1000].TraceMetaData.source_location_x << endl;
  delete (seg);
}

void SEGYIOManager::WriteTracesDataToFile(string file_name, string sort_type,
                                          SeIO *sio) {
  SUSegy *seg = new SUSegy();
...
void SEGYIOManager::WriteVelocityDataToFile(string file_name, string sort_type,
                                            SeIO *sio) {
...
void SEGYIOManager::WriteDensityDataToFile(string file_name, string sort_type,
                                           SeIO *sio) {
