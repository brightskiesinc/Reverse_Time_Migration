...
vector<uint> SUSegy::GetUniqueOccurences(string filename, vector<SEGYelement> *select_element, uint min_threshold, uint max_threshold) {
  if (!file.is_open())
...
void SUSegy::ReadBinaryHeader(string filename, bool exit_on_error) {
...
void SUSegy::ReadHeadersAndTraces(
  ...
  while (itr < INT_MAX) {
    ...
    if (!file) { // error in reading (reaching the end of the file)
      break;
      cout << "reaching end of the file" << endl;
    }
    tapesegy_to_segy(&tapetrace, &trace); // converts to easily readable format
                                          // (oblivious to the type int/short)
    if (endian == 0)
