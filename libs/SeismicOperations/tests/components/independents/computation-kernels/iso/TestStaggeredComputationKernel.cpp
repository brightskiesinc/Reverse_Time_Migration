...
void TEST_CASE_STAGGERED_COMPUTATION_KERNEL(GridBox *apGridBox,
    ...
    auto pressure_next = new FrameBuffer<float>();

    auto particle_vel_x = new FrameBuffer<float>();
    auto particle_vel_z = new FrameBuffer<float>();

    auto velocity = new FrameBuffer<float>();
    auto density = new FrameBuffer<float>();
    ...
    particle_vel_z->Allocate(window_size);

    velocity->Allocate(size);
    density->Allocate(size);

    apGridBox->RegisterWaveField(WAVE | GB_PRSS | CURR | DIR_Z, pressure_curr);
    apGridBox->RegisterWaveField(WAVE | GB_PRSS | NEXT | DIR_Z, pressure_next);

    apGridBox->RegisterWaveField(WAVE | GB_PRTC | CURR | DIR_Z, particle_vel_z);
    apGridBox->RegisterWaveField(WAVE | GB_PRTC | CURR | DIR_X, particle_vel_x);


    apGridBox->RegisterParameter(PARM | GB_VEL, velocity);
    apGridBox->RegisterParameter(PARM | GB_DEN, density);
    ...
    for (int iy = 0; iy < ny; iy++) {
        for (int iz = 0; iz < nz; iz++) {
            for (int ix = 0; ix < nx; ix++) {
                ...
                temp_den[iz * nx + ix + (iy * nx * nz)] = 1.0f;
            }
        }
    }

    Device::MemSet(pressure_curr->GetNativePointer(), 0.0f, window_size * sizeof(float));
    Device::MemSet(pressure_next->GetNativePointer(), 0.0f, window_size * sizeof(float));

    Device::MemSet(particle_vel_z->GetNativePointer(), 0.0f, window_size * sizeof(float));
    Device::MemSet(particle_vel_x->GetNativePointer(), 0.0f, window_size * sizeof(float));
    ...
    if (ny > 1) {
        ...
        auto particle_vel_y = new FrameBuffer<float>();
        particle_vel_y->Allocate(window_size);
        apGridBox->RegisterWaveField(WAVE | GB_PRTC | CURR | DIR_Y, particle_vel_y);
        Device::MemSet(particle_vel_y->GetNativePointer(), 0.0f, window_size * sizeof(float));


    }

    /*
     * StaggeredComputationKernel setting.
     */

    auto computation_kernel = new StaggeredComputationKernel(apConfigurationMap);
...
TEST_CASE("Staggered Order - 2D - Window", "[Window],[2D]") {
    TEST_CASE_STAGGERED_COMPUTATION_KERNEL(
