...
void SeismicTraceManager::ReadShot(vector<string> filenames, uint shot_number, string sort_key) {
  ...
  float scale = std::abs(sio->Atraces.at(0).TraceMetaData.scalar) * 1.0;
  ...
  //cout << "sample nt " << sample_nt << std::endl;
  ...
  int iz_start = std::max(0, r_start.z);
  int ix_start = std::max(0, r_start.x);
  int iy_start = std::max(0, r_start.y);

  int iz_end = std::min(r_end.z, (int)grid->grid_size.nz);
  int ix_end = std::min(r_end.x, (int)grid->grid_size.nx);
  int iy_end = std::min(r_end.y, (int)grid->grid_size.ny);
  ...
  grid->nt = int(sample_nt * traces->sample_dt /
                 grid->dt); // we dont have total time , but we have the nt from
                            // the segy file ,
  // so we can modify the nt accourding to the ratio between the recorded dt and
  // the suitable dt
...
void SeismicTraceManager::ApplyTraces(uint time_step) {
  ...
  int r_start_y = r_start.y;
  int r_start_x = r_start.x;
  int r_start_z = r_start.z;
  int trace_nx = traces->num_receivers_in_x;
  int ele_x = (r_end.x - r_start.x) / x_inc;
  int ele_y = (r_end.y - r_start.y) / y_inc;

  AcousticDpcComputationParameters::device_queue->submit([&](handler &cgh) {
    auto global_range = range<3>(1, ele_y, ele_x);
