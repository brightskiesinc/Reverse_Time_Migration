...
void do_compression_save(float *array, int nx, int ny, int nz, int nt, double tolerance,
  ...
  switch (codecType) {
      ...
      break;
      case 2:
          compressZFP_Parallel(array, nx, ny, nz, tolerance, compressed_file, zfp_is_relative);
          break;
      default:
...
void do_decompression_load(float *array, int nx, int ny, int nz, int nt, double tolerance,
  ...
  switch (codecType) {
      case 1:
          ...
          break;
      case 2:
          decompressZFP_Parallel(array, nx, ny, nz, tolerance, compressed_file, zfp_is_relative);
          break;
      default:
          fprintf(stderr, "***Invalid codec Type, Terminating\n");
          exit(0);
